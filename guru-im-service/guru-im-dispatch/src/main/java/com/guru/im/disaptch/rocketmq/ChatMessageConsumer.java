 ...
 */
@Service
@RocketMQMessageListener(topic = MQTopic.DISPATCH_CHAT_TOPIC,
        consumerGroup = "${rocketmq.consumers.chat-group}",
        consumeMode = ConsumeMode.CONCURRENTLY,
        messageModel = MessageModel.CLUSTERING)
public class ChatMessageConsumer implements RocketMQListener<MQMessageWrapper> {
    private final IMMessageListener messageListener;

    public ChatMessageConsumer(IMMessageListener messageListener) {
